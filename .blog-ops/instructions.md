...
**话题多样性（由 AI 判断，非关键词数组）**：
- 历史教训：早期用标题关键词重叠判断重复，"AI科学家"和"AI人才战争"关键词重叠低被放过，导致连续几篇都扎堆 AI 领域。
- 现在的做法：每次选题前，用免费的 Qwen3-8B 把最近 3 篇文章标题归类到大领域（体育/文化娱乐/技术折腾/国际/财经/AI科技/生活随笔等，领域由 AI 自行判断），并让它指出是否有领域偏多（saturated）。
- 若存在 saturated 领域：
  - 搜索路径：把 saturated 领域 + 最近分类分布注入选题 prompt，要求 AI 必须选其他领域；Tavily 热点搜索的 focus 也会每次轮换（财经/国际/体育/文化/社会/科技），避免每次都涌向 AI/科技。
  - 选题池路径：再用 AI 判断池内选题是否属于 saturated 领域，是则回退到搜索路径。
- 选题完成后还有一次 AI 校验：若最终话题仍落在 saturated 领域，会打印 WARNING 提示人工关注。
- 不维护任何关键词数组——领域判断完全交给 AI，对新话题自适应。
